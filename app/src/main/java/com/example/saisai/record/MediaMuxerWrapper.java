...
public class MediaMuxerWrapper {
    ...
    /**
     * request readyStart recording form encoder
     * @return true when muxer is ready to write
     */
    /*package*/synchronized boolean start(){
        if(DEBUG) Log.v(TAG,"readyStart:");
        mStatredCount++;
        if((mStatredCount>0)&&(mStatredCount==mEncoderCount)){
            mMediaMuxer.start();
            mIsStarted=true;
            notifyAll();
    ...
    /*package*/synchronized void stop(){
        ...
        mStatredCount--;
        if((mStatredCount>0)&&(mStatredCount<=0)){
            mMediaMuxer.stop();
            mMediaMuxer.release();
            mIsStarted=false;
    ...
    /*package*/ synchronized int addTrack(final MediaFormat format){
        ...
        if(DEBUG) Log.i(TAG,"addTrack:trackNum="+mEncoderCount+",trachIx="+trackIx+",format="+format);
        return trackIx;
    }
