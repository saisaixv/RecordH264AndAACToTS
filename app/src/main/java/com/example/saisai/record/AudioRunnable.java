...
public class AudioRunnable extends Thread {
    ...
    protected static final int TIMEOUT_USEC=10*1000;//10[msec]
        ...
        while (!isExit){
            ...
            if(!isStart){
                ...
                if(!isMuxerReady){
                    synchronized (lock){
                        try {
                            if(DEBUG) Log.e("angcyo-->","audio -- 等待混合器准备...");
                            lock.wait();
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        }
...
//                    muxer.addMuxerData(new MediaMuxerRunnable.MuxerData(MediaMuxerRunnable.TRACK_AUDIO,encodedData,mBufferInfo));

                    prevOutputPTSUs=mBufferInfo.presentationTimeUs;
                    byte[] output=new byte[mBufferInfo.size];
                    byte[] temp=new byte[mBufferInfo.size];
                    encodedData.get(output);
...
//                Log.e(TAG,"pts = "+mBufferInfo.presentationTimeUs);
